# Changelog

本文件记录 AppSnapshoter 各版本的面向用户变更。格式参考 [Keep a Changelog](https://keepachangelog.com/zh-CN/)，版本号遵循 [语义化版本](https://semver.org/lang/zh-CN/)。

## [Unreleased]

## [1.3.0] - 2026-08-23

### Added

- 存档 Tab 搜索：按分组集、分组或应用名过滤，匹配项高亮；搜索时折叠/展开与跳转行为与平时隔离
- 应用 Tab 长按弹出菜单：查看已加入的分组并跳转存档 Tab；可将应用加入分组（含分组集内分组，显示为「集名 / 组名」）；支持打开系统应用信息、应用配置与卸载

### Changed

- 应用 Tab 长按归属由对话框改为与存档 Item 同构的弹出菜单

### Fixed

- 应用 Tab 在应用列表尚未加载完成时不再误显示空白列表
- 加入分组后「未分组」筛选会正确更新，应用不再残留在筛选结果中
- 存档 Tab 搜索状态下跳转分组、退出搜索后组内排序等边界行为

## [1.2.1] - 2026-08-22

### Changed

- 创建快照前仅挂起应用，不再强制停止，降低部分应用备份时 APK 无法打包的风险

### Fixed

- 快照某一步失败时不再当作成功关掉进度框：会中止后续步骤、删除半成品，并提示错误
- 单应用创建快照成功或失败时显示 Toast 提示

## [1.2.0] - 2026-08-21

### Added

- 分组集：用父目录组织多个分组，存档列表按集成块显示，支持折叠/展开、两级排序与强调色
- 长按底栏「存档」可弹出分组集列表，点选或拖选跳转到对应集
- 分组可设为独占或共享归属；冲突时提示，支持在组间移动应用
- 选择应用时可按「未分组」筛选；添加入口统一支持添加分组或分组集

### Changed

- 分组集标题栏改为强调色描边，背景与分组卡片一致；滚动时可吸顶
- 组内应用网格超过约三行后在组内滚动，避免整页被撑开
- 空分组始终显示加号入口；有应用时才出现折叠箭头
- 展开/折叠分组集与一键折叠更快，不再整表重扫磁盘

### Fixed

- 应用列表与选择应用对话框的加载指示更准确
- 加固应用冻结/解冻链路；修复 split APK 安装 session ID 解析失败

## [1.1.1] - 2026-06-18

### Added

- 全面中英双语：界面文案支持简体中文与英文（跟随系统语言）

### Changed

- 应用 Tab 筛选行改为用户 Tab + 系统/用户图标切换 + 搜索图标，与选应用/忽略应用页共用
- 筛选图标触控区与居中显示优化
- 标签筛选支持单行横向滚动与展开/收起；Tag Chip 尺寸缩小

### Fixed

- 应用 Tab 标签无法选中；标签展开/收起时崩溃
- 应用 Tab 标签列表因分组名称未加载导致的崩溃
- 时间线列表项长按无法进入多选
- Root 服务未连接时应用崩溃
- 文件系统服务初始化可能无限等待（现增加超时）
- 时间线页观察者重复注册导致的内存泄漏
- 归档应用在信息未加载完成时查询运行状态崩溃
- 原生文件操作中的内存泄漏

## [1.1.0] - 2026-06-18

### Added

- 时间线 Tab：按时间范围跨分组浏览快照，支持多选批量恢复、删除与导出
- 快照目录写入 `.nomedia`，避免图库扫描应用图标缓存
- 悬浮底栏支持分组批量恢复与归档

### Changed

- 刷新主界面 UI（Fluent 2 主题、紧凑顶栏、悬浮毛玻璃底栏；时间线/应用 Tab 支持可折叠搜索）
- 用自定义悬浮底栏替代系统 BottomNavigationView，改善横屏切换时的布局问题

### Fixed

- 添加分组后归档列表不立即显示新分组
- 添加应用到分组后列表与「全部归档」偶发不同步；空分组点击添加应用无法弹出选应用对话框
- 批量还原对话框因损坏的还原记录崩溃
- 设置页缺少顶栏，与主界面风格不一致
- 锁定竖屏，避免旋转屏幕时对话框与进度框丢失
- 多用户环境下额外排除模式文件选择器路径错误
- 分组组头在真机上标题与按钮布局异常、触控区过小

## [1.0] - 2026-05-21

### Added

- 一键存档 / 一键恢复：长按应用快速创建快照并还原
- 基于 JNI 的 TAR + ZSTD 压缩管道，支持 FIFO 流式打包
- APK 智能去重、多存档管理、自定义压缩目录
- 分组管理：按组批量快照与恢复
- Root 服务架构（AIDL + libsu），UI 与 Root 逻辑隔离
- 保留策略与排除策略配置 UI
