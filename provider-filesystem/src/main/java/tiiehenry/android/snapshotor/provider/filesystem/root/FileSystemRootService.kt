...
import tiiehenry.android.snapshotor.provider.filesystem.compressors.zstd.CountingOutputStream
...
class FileSystemRootService : RootService() {
    ...
    private class Impl(private val context: Context) : IFileSystemRootService.Stub() {
        ...
        fun onBind() {
            ...
            mWifiManager = mSystemContext.getSystemService(WIFI_SERVICE).castTo()
        }
        ...
        override fun setUid(path: String, uid: Int): Boolean {
            return runCatching {
                Os.chown(path, uid, -1) // -1 means don't change gid
                true
            }.getOrNull() ?: false
        }
        ...
        override fun extractTar(tarFifo: String, targetDir: String): Boolean {
            return runCatching {
                ...
                Log.e("FileSystemRootService", "Failed to extract tar", exception)
            }.getOrNull() ?: false
        }
    }
