...
| `SnapshotViewModel` | 全局单例 ViewModel；`groupList` / `appsList` / `isAppsLoading` / `isAppsCatalogLoaded` 来自 `AppDataRepository` LiveData；数据变更委托 repository 进程级协程（不用 `viewModelScope`） |
...
| 类 | 职责 |
|---|------|
| `AppsFragment`, `BaseAppsFragment` | 应用列表 Fragment；长按 `AppsItemPopupMenu` 见 [应用 Tab Item 长按 Popup](../../systems/snapshot/APPS_TAB_ITEM_POPUP.md) |
| `AppsAdapter`, `AppsListComponent`, `AppsItemPopupMenu`, `AppsPopupGroupAdapter` | 列表与 popup；`groupList` 变化重跑 membership 筛选 |
...
| 类 | 职责 |
|---|------|
| `AppDataRepository` | 单例仓库；`scope`（`SupervisorJob + IO`）执行 `loadGroups` / `addGroup` / `deleteGroup`；`loadGroupsMutex` 防并发 stale；`groupList` / `appsList` / `isAppsLoading` / `isAppsCatalogLoaded` LiveData。`isAppsLoading` 为拉取中；`isAppsCatalogLoaded` 为已成功拿到用户列表（与 `appsList` 初值 `emptyMap()` 区分「尚未加载」）。`loadData` 在 `loadGroups` 前即置 loading true；`loadApps` 在用户列表为空时不标记 loaded（Root 未就绪/IPC 失败），UI 保持 loading 并可 `scheduleLoadApps` 重试。UI 禁止用 `appsList` 排放驱动 loading |
