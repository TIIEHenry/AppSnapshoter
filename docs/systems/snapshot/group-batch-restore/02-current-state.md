...
| 能力 | 实现 | 说明 |
|------|------|------|
| 单应用恢复最新 | `ArchiveRestorer.restoreLatest()` | `LauncherViewModel.onGroupItemClicked()` 调用 |
| 单应用恢复指定快照 | `ArchiveRestorer.restoreArchiveItem()` | 应用弹窗菜单选具体 `ArchiveItem` |
...
| `GroupActionsController` | `main/launch/GroupActionsController.kt` | 组头按钮事件；`btn_archive_all` 触发归档 |
...
当前结构：**单行** — 标题（18sp）与 5 个 `40×40dp` 按钮并列：

| 按钮 | id | 功能 |
|------|-----|------|
| 确认排序 | `btn_confirm` | 自定义排序模式（通常 gone） |
| 刷新 | `btn_refresh` | 重载组内应用；长按显示统计 |
| 添加 | `btn_add` | 添加应用到组 |
| 排序 | `btn_move` | 排序方式 PopupMenu |
| 设置 | `btn_tune` | 分组配置 |
| 全部归档 | `btn_archive_all` | 批量创建快照 |

**空间估算（360dp 屏宽）：**

- 5 个可见按钮 ≈ 5 × (40 + 8) = **240dp**
...
| 快照选取（新/旧） | `TimelineRepository.resolveArchive()` + `RestoreStrategy` | 扩展为 `ArchivePickStrategy` |
| 挂起恢复 | `ArchiveRestorer.restoreArchiveSuspend()` | 批量循环调用 |
| 安装状态判断 | `AppStatusHelper.isAppInstalled()` | 范围过滤 |
| 策略对话框布局 | `dialog_restore_strategy.xml` | 参考样式，新建组合对话框 |
| 防重复提交 | `TimelineViewModel.isBatchRunning` | 在 `LauncherViewModel` 增加等价标志 |
...
| 运行时解析 | `TimelineRepository.resolveEntry(key, groups, range)` | 直接从 `group.apps` 构建 plan；执行前建议重查 `groupList` 防 stale |
