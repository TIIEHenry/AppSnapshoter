...
- 为批量恢复腾出入口，**不增加独立按钮数量**（`btn_archive_all` → `btn_batch` 菜单）
- 长组名可读（单行省略，不被图标挤没）
- 保持与现有 Material 图标风格一致

### 布局结构

保持 **单行**：标题与工具栏并列，标题 `layout_weight=1` 占据剩余宽度。
...
|------|--------|--------|
| 标题字号 | 18sp | **17sp** |
| 标题行 | 与按钮同行 | **与按钮同行**，`layout_weight=1`，`maxLines=1`，`ellipsize=end` |
| 图标按钮 | 40×40dp，margin 4dp，padding 8dp | **36×36dp**，margin **3dp**，padding **7dp** |
| 工具栏 | 与标题同一 `LinearLayout` | 同行 `horizontal`，`gravity=center_vertical` |

> 首版曾尝试双行 + 32dp 以缓解宽度压力；真机反馈标题与按钮分离、触控区偏小后，改回单行并取 17sp / 36dp 折中尺寸。

### 按钮映射

| id | 图标 | 功能 | 变更 |
|----|------|------|------|
| `group_title` | — | 点击折叠/展开；长按分组设置 | 保留，与按钮同行，`layout_weight=1` |
| `btn_confirm` | check | 确认自定义排序 | 保留，同行 |
...
将 **排序**、**设置** 收入 `btn_more`（⋮）溢出菜单，工具栏仅保留 `[↻] [+] [批量] [⋮]`。当前单行 + 36dp 在常见屏宽下可接受；真机若仍拥挤再启用溢出。
