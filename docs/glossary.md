...
| **独立组 (Independent Group)** | 不属于任何分组集的 `SnapGroup`（存档列表 `GroupCard.setId == null`）。应用 Tab 长按「加入」v1 只针对这类组。见 [应用 Tab Item 长按 Popup](systems/snapshot/APPS_TAB_ITEM_POPUP.md) |
...
| **应用 catalog** | 已安装应用列表（按用户分组）。`isAppsCatalogLoaded` 表示已成功拉取用户列表；初值空 Map 不是「已加载」。见应用 Tab loading 契约 |
