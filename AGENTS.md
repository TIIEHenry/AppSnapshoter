...
**ViewModels**: `SnapshotApp` instantiates `SnapshotViewModel` directly in `onCreate()` (not via `ViewModelProvider`) and exposes it as a top-level property via `SingletonViewModelFactory`. **Do not use `viewModelScope` on `SnapshotViewModel` for data loading** — Activity `onCleared()` cancels the scope while the singleton instance survives; group/app list mutations (`loadGroups`, `addGroup`, `deleteGroup`, `addAppsToGroup`) run on `AppDataRepository`'s process-level `CoroutineScope` (`SupervisorJob + Dispatchers.IO`) with `loadGroupsMutex` for serial reloads. `AppsViewModel` filters the app list from `SnapshotViewModel` using multi-dimensional filters. `TimelineViewModel` queries in-memory snapshots from `groupList` by time range; `navigateToGroup` on `SnapshotViewModel` scrolls the archive tab to a group when invoked from the timeline tab. **Apps catalog loading SSOT** is `AppDataRepository.isAppsLoading` + `isAppsCatalogLoaded` (exposed via `SnapshotViewModel`). `AppsListComponent` shows loading as `!catalogLoaded || isAppsLoading || isLocalProcessing`. Initial `appsList=emptyMap()` is **not** a loaded catalog — do not hide the ProgressBar until a user list was fetched. While visible, if catalog is still unloaded after a fetch, retry up to `AppsCatalogUi.MAX_VISIBLE_ATTEMPTS` with backoff. Resolve `fileSystem` / `users` on IO, not the main thread. **Do not drive apps-list ProgressBar from `appsList` emissions**. `loadApps()` refreshes only the installed catalog; `loadData()` still does groups then apps. Same invariant as Timeline's separate `isQuerying` vs data LiveData.
